    ...
    void ToGraph(const IxIndexHandle *ih, IxNodeHandle *node,
        ...
        if (node->is_leaf_page()) {
            ...
            for (int i = 0; i < leaf->get_size(); i++) {
                out << "<TD>" << *reinterpret_cast<int *>(leaf->get_key(i))
                    << "</TD>\n";
            }
            out << "</TR>";
            // Print table end
            ...
            if (leaf->get_next_leaf() != INVALID_PAGE_ID &&
                leaf->get_next_leaf() > 1) {
    ...
    void check_tree(const IxIndexHandle *ih, int now_page_no) {
        ...
        for (int i = 0; i < node->get_size(); i++) {  // 遍历node的所有孩子
            ...
            if (i + 1 < node->get_size()) {
                // 满足制约大小关系
                ASSERT_LT(child_last_key,
                          node->key_at(
                              i + 1));  // child_last_key < node->KeyAt(i + 1)
            }
...
TEST_F(BPlusTreeTests, InsertAndDeleteTest1) {
    ...
    for (auto key : delete_keys) {
        ...
        // Draw(buffer_pool_manager_.get(), "InsertAndDeleteTest1_delete" +
...
TEST_F(BPlusTreeTests, InsertAndDeleteTest2) {
    ...
    for (auto key : delete_keys) {
        ...
        // Draw(buffer_pool_manager_.get(), "InsertAndDeleteTest2_delete" +
        // std::to_string(key) + ".dot");
    }
}
