...
TESTS = ["commit_test", "abort_test", "commit_index_test", "abort_index_test"]
...
def run_test(test_case):
    ...
    try:
        ...
        comment_str += (
            "In Transaction unit test:"
            + str(test_case)
            + ", open standard_answer failed\n"
        )
        subprocess.run(
    ...
    try:
        ...
        FAILED_TESTS.append(test_case)
        comment_str += (
            "In Transaction unit test:" + str(test_case) + ", cannot open output file\n"
        )
        subprocess.run(
            "ps -ef | grep rmdb | grep -v grep | awk '{print $2}' | xargs kill -9",
            shell=True,
    ...
    if len(FAILED_TESTS) != 0:
        print(
            f"\033[0;31;40mTransaction Test {test_case} Final Score:"
            + str(score)
            + "\033[0m"
        )
